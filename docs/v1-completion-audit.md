...
| 3. 档案与模拟设备 | 已完成 | 首次引导、邀请接收、模拟发现/配网/在线/离线/激活、健康档案、音量/朗读/勿扰、解绑与退出家庭；onboarding E2E 测试。 |
| 4. 计划与每日任务 | 已完成 | 三步创建，七类计划，暂停/恢复/结束，时区生成任务，周/月/历史详情，完成/没做与幂等键，手动提醒；双端仓库与 E2E 测试。 |
| 5. 健康、火花与告警 | 已完成 | 血压/血糖/心情/体重确认记录、趋势入口、L1/L2 告警与 24 小时去重、处理/关闭、连续火花与历史；L3 保持禁用。 |
| 6. 话题、消息、设置与隐私 | 已完成 | 模板话题、复制事件、微信可用时打开微信/否则系统分享、四类消息、周报、双端设置入口、数据导出、反馈与 30 天注销执行器。 |
...
- Flutter：`flutter analyze --no-pub` 无问题；54 项测试通过，包含双端流程、离线、390×844 Golden 与 360/390/430 宽度；Web Debug 生产环境构建成功。
- API：Prisma schema 校验与 Client 生成成功；10 个测试套件、25 项测试通过；NestJS 构建成功。
- 隐私：测试覆盖预约注销、到期执行、旧 Access Token 失效，以及原手机号可重新注册。
- 静态发布文件：`Info.plist` XML 校验通过；本轮源码无空 `onTap`/`onPressed` 回调。
