...
describe('Topics, messages, feedback, and privacy', () => {
  ...
  beforeAll(async () => {
    ...
    token = login.body.data.accessToken; await request(app.getHttpServer()).post('/api/families').set('Authorization', `Bearer ${token}`).expect(201);
  });
  ...
  it('submits feedback, exports data, and schedules deletion', async () => {
